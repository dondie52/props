...
export default function Page() {
  ...
  const [messageTarget, setMessageTarget] = useState<TenantRow | null>(null);
  ...
  useEffect(() => {
    const loadTenants = async () => {
      const { data } = await supabase
        .from("tenants")
        .select("id,full_name,email,lease_end,units(id,property_id,unit_number,rent_amount,properties(id,name))");

      const now = new Date();
      const mapped: TenantRow[] = (data ?? []).map((tenant) => {
        const tenantObj = tenant as unknown as { id?: unknown; full_name?: unknown; email?: unknown; lease_end?: unknown; units?: unknown };
  ...
  const openMessageModal = (tenant: TenantRow) => {
    setMessageTarget(tenant);
    setMessageBody("");
    setMessageError("");
  ...
  const onSendMessage = async () => {
    ...
    try {
      ...
      const { conversationId } = await getOrCreateConversation({
        tenantEmail: messageTarget.email,
        tenantId: messageTarget.tenantId,
        propertyId: messageTarget.propertyId,
  ...
  return (
    <div className="flex min-h-screen bg-bg-page">
                    ...
                    <td>
                      <div className="flex items-center gap-2 text-text-muted">
                        ...
                        <button
                          ...
                          onClick={() => openMessageModal(tenant)}
                        >
      ...
      <Modal isOpen={isMessageModalOpen} onClose={() => setIsMessageModalOpen(false)} title={`Message ${messageTarget?.name ?? "tenant"}`}>
        <div className="space-y-3">
          <p className="text-xs text-text-muted">
            This sends an in-app message thread for {messageTarget?.property ?? "-"} · Unit {messageTarget?.unit ?? "-"}.
          </p>
          <textarea
            rows={5}
            value={messageBody}
            ...
            className="w-full rounded-base border border-border-ghost px-3 py-2 text-sm focus:border-primary focus:outline-none"
          />
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              className="h-10 rounded-base border border-border-ghost px-4 text-sm text-text-sub"
            >
              Cancel
            </button>
            <button
              ...
              className="h-10 rounded-base bg-accent px-4 text-sm font-semibold text-white disabled:opacity-70"
            >
