  ...
  useEffect(() => {
    const loadThreads = async () => {
      ...
      const { data: conversationRows, error: conversationsError } = await supabase
        .from("conversations")
        .select("id,last_message_at,tenant_profile_id,property_id,unit_id")
      ...
      const tenantProfileIds = Array.from(new Set((conversationRows ?? []).map((row) => row.tenant_profile_id)));
      ...
      const [{ data: tenantProfiles }, { data: properties }, { data: units }] = await Promise.all([
        tenantProfileIds.length ? supabase.from("profiles").select("id,full_name").in("id", tenantProfileIds) : Promise.resolve({ data: [] }),
      ...
      const tenantMap = new Map((tenantProfiles ?? []).map((row) => [row.id, row.full_name]));
      ...
      const mappedThreads = (conversationRows ?? []).map((row) => ({
        id: row.id,
        tenantName: tenantMap.get(row.tenant_profile_id) ?? "Tenant",
        ...
        <main className="flex gap-6 p-8 pt-24">
          ...
          <Card className="flex-1 p-5">
            <div className="border-b border-border-ghost pb-3">
              <h2 className="text-lg font-semibold text-primary">{activeThread?.tenantName ?? "Select a conversation"}</h2>
              <p className="text-xs text-text-muted">
                {activeThread ? `${activeThread.propertyName} · Unit ${activeThread.unitNumber}` : "Use Tenants page to start a new chat."}
              </p>
            </div>
