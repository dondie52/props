...
export default async function Page() {
  ...
  if (scope.landlordId && scope.tenantIds.length > 0) {
    const [{ data, error: paymentsError }, { data: tenantData, error: tenantsError }] = await Promise.all([
      supabase
        .from("payments")
    ...
    if (paymentsError) {
      loadError = paymentsError.message;
      console.error("payments page query", paymentsError);
    } else {
      rows = (data ?? []).map((row) => {
    ...
    if (tenantsError) {
      loadError = loadError ? `${loadError}; ${tenantsError.message}` : tenantsError.message;
      console.error("payments page tenants query", tenantsError);
    } else {
  ...
  return <PaymentsClient initialRows={rows} tenantOptions={tenantOptions} loadError={loadError} />;
}
