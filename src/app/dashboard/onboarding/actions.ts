...
function debugOnboardingServerLog(
  location: string,
  message: string,
...
async function getCurrentProfile(supabase: ReturnType<typeof createSupabaseServerActionClient>) {
  ...
  debugOnboardingServerLog(
    "src/app/dashboard/onboarding/actions.ts:getCurrentProfile:getUser",
    "Fetched auth user for onboarding",
    { hasUserId: Boolean(user?.id), hasEmail: Boolean(email) },
    "H1",
  );
  ...
  let { data: profile, error } = await supabase
    ...
    .maybeSingle();
  debugOnboardingServerLog(
    "src/app/dashboard/onboarding/actions.ts:getCurrentProfile:primaryProfileQuery",
    "Primary profile query completed",
    { hasProfile: Boolean(profile?.id), hasError: Boolean(error), errorMessage: error?.message ?? null },
    "H2",
  );
  if (!profile && !error) {
    ...
    debugOnboardingServerLog(
      "src/app/dashboard/onboarding/actions.ts:getCurrentProfile:fallbackProfileQuery",
      "Fallback profile query completed",
      { hasProfile: Boolean(profile?.id), hasError: Boolean(error), errorMessage: error?.message ?? null },
      "H3",
    );
  }
