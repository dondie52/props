...
function debugOnboardingClientLog(
  location: string,
  message: string,
  ...
  fetch("http://127.0.0.1:7851/ingest/cfa3b922-9c8a-4c92-bfaf-18b672b0b00c", {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Debug-Session-Id": "bbb654" },
    body: JSON.stringify({
      sessionId: "bbb654",
      location,
      message,
...
export default function OnboardingWizard() {
  ...
  const onCreateProperty = () => {
    setError("");
    debugOnboardingClientLog(
      "src/components/onboarding/OnboardingWizard.tsx:onCreateProperty",
      "Create property clicked",
      {
        propertySetupMode,
        pending,
        hasName: Boolean(property.name.trim()),
        hasAddress: Boolean(property.address.trim()),
    ...
    startTransition(async () => {
      try {
        ...
        if (propertyPhotos.length > 0) {
          ...
          formData.set("propertyId", result.propertyId);
          for (const photo of propertyPhotos) formData.append("photos", photo);
          await uploadPropertyPhotosAction(formData);
        }
        ...
        debugOnboardingClientLog(
          "src/components/onboarding/OnboardingWizard.tsx:onCreateProperty:catch",
                ...
                <div className="md:col-span-2">
                  <Field label="Pictures (optional)">
                    <input
                      ...
                      accept="image/*"
                      multiple
                      className="w-full rounded-base border border-border-ghost bg-white px-3 py-2 text-sm file:mr-3 file:rounded-base file:border-0 file:bg-primary file:px-3 file:py-2 file:text-sm file:font-semibold file:text-white"
                      onChange={(e) => setPropertyPhotos(Array.from(e.target.files ?? []))}
                    />
                    <p className="mt-1 text-xs text-text-muted">
                      Upload up to 8 images (max 5MB each). The first image becomes the cover photo.
                    </p>
                    {propertyPhotos.length ? (
                      <p className="mt-1 text-xs text-text-sub">
                        Selected: {propertyPhotos.map((photo) => photo.name).join(", ")}
                      </p>
                    ) : null}
                  </Field>
                </div>
